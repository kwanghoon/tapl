...
let stringBuffer = ref (Bytes.to_string (Bytes.create 2048))
let stringEnd = ref 0
...
in
  if x = String.length buffer then
    begin
      let newBuffer = (Bytes.to_string (Bytes.create (x*2))) in
      String.blit buffer 0 (Bytes.of_string newBuffer) 0 x;
      String.set (Bytes.of_string newBuffer) x ch;
      stringBuffer := newBuffer;
      stringEnd := x+1
    end
  else
    begin
      String.set (Bytes.of_string buffer) x ch;
      stringEnd := x+1
    end
